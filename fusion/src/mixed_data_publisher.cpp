...
#include "sensor_msgs/msg/nav_sat_fix.hpp"  // Include the header for NavSatFix message type
...
public:
    MixedDataPublisher() : Node("mixed_data_publisher"), data_publishing_done_(false)  // Constructor initializes the node with the name "mixed_data_publisher" and sets the flag to false
    {
        ...
        std::string merged_file_path = "/home/mooo/aub/datasets/ficosa_for_HSLAM/Merged_results/merged_output_11.txt";
        ...
        ekf_data_file_.open("/home/mooo/aub/datasets/ficosa_for_HSLAM/Results_EKF/ekf_result_11.txt", std::ios::out | std::ios::app);
...
private:
    ...
    void loadAndPublishData(const std::string &file_path)
    {
        ...
        for (size_t i = 0; i < lines.size(); ++i) {
            ...
            if (id == 0) {
                ...
                if (i + 1 < lines.size()) {
                    ...
                    if (next_id == 1) {
                        ...
                        next_iss >> latitude >> longitude >> altitude;

                        // Publish both H-SLAM and GPS data
                        publishHSLAMData(timestamp, x, y, z);
                        publishGPSData(next_timestamp, latitude, longitude, altitude);
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds(1));  // 10 milliseconds
        }
    ...
    void publishGPSData(double timestamp, double latitude, double longitude, double altitude)
    {
        ...
        RCLCPP_INFO(this->get_logger(), "Published GPS data: %.0f %.8f %.8f %.3f", timestamp, latitude, longitude, altitude);
    }
